...
SamplerTrack::mix2(float** dest, size_t spiaz , size_t pos , size_t framecount,float note_vol)
{
	...
	if((res_enable || curNote->getNote()!=60) && coef!=0)
	{
...
SamplerTrack::NoteReady(int notenumber,int patternnumber,float samplesperbeat)
{
	...
	if(n->getValue())
	{
			...
			if(curSample==NULL) { note_status=0; return ; }
			
			if(need_res || ( samplesperbeat!=this->samplesperbeat && res_enable) )
			{
			...
			if(!res_enable) {
					if(n->getNote()!=60)
					{
					coef=pitchtable[60]/pitchtable[n->getNote()];
					converter->initialize(coef,2);
					virtualbytes=(uint32)floor((float)((float)curSample->totalbytes/8.)*coef)*8+1;
					}
					else
			...
			/*
			if(curSample!=NULL && (need_res || samplesperbeat!=this->samplesperbeat || n->getValue()>0)  )
			{
				this->samplesperbeat=samplesperbeat;
				if(res_enable)
				{

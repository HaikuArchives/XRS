...
{
	//JTrack::RControl();
	AddChild(sw=new SampleView(BRect(SBUTTON_W,0,300,Frame().Height())) );	
...
JAnalizerTrack::Reset(Pattern* p ,Track* tr)
{
	//JTrack::Reset(p,tr);
	// SetName("Analizer");
	((AnalizerTrack*)tr)->jat=this;
}

void
JAnalizerTrack::DrawBuffer(float** buffer,int32 samples_num,int spiaz)
{
	if(Window()->Lock()){
	sw->Init(buffer,samples_num,spiaz);
	Window()->Unlock();
	}
}
void
JAnalizerTrack::Select(){}

void
JAnalizerTrack::Deselect(){}

...
void TB303Knobs::AttachedToWindow()
{
	...
	((PlugWindow*)window)->SetPrograms("[ Tn306 ]",NULL);
...
TB303Knobs::MakeController(BRect r,const char* n1,const char* n2,int min,int max ,int cur,const rgb_color col,int32 ms)
{
	...
	p=new BPot(BRect(r.left,r.top,r.left+POT_H,r.top+POT_H), "","",info,NULL,min,max,::MakeBitmapSkin("tnpot"),NULL, B_FOLLOW_LEFT_RIGHT | B_FOLLOW_TOP,B_WILL_DRAW|B_FRAME_EVENTS|B_NAVIGABLE_JUMP);
	p->SetTarget(this);
	p->SetHighColor(col);

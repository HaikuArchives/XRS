...
{
	//if(async!=NULL){ async->SetMeter(buffer[0][spiaz]);}
	...
	if(async!=NULL){ async->SetMeter((float)max/32767.*curNote->getVolume()*vol_left*1.5,(float)max/32767.*vol_right*curNote->getVolume()*1.5);}
}
...
void SpecialTrack::Message(SynthMessage msg, float data)
{
	...
	switch(msg) {
		case TempoChange: 
			...
			break;
		case SystemStop:
		if(async!=NULL) async->SetMeter(0,0);
		break;
		case SystemReset:
			n=(int)data;
...
void
SpecialTrack::goOff()
{ re_synch=true;  	if(async!=NULL) async->SetMeter(0,0);}

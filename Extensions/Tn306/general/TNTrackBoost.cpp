...
TNTrackBoost::Load(Track* cur,int t,BFile* file,int32 va,int32 rt)
{
	...
	switch(va){
			...
			case TRACK_SPECIFIC_DATA_START:
				...
				//win
				if(msg->FindFloat("TN_winx",&(st->winx))==B_OK)
				st->winy=msg->FindFloat("TN_winy");	
				else
				{
					//st->async->SetConfig(msg);	
					//printf("New zistem!\n");
					st->winx=msg->FindFloat("Xpos");
					st->winy=msg->FindFloat("Ypos");
					st->big=msg->FindBool("big");
					st->hidden=msg->FindBool("hidden");
					st->Ysize=msg->FindFloat("Ysize");
					printf("New zistem %f!\n",st->Ysize);
				}						
...
TNTrackBoost::Save(Track *cur,int t,BFile* file,int mea)
{
	...
	st=(SpecialTrack*)cur;
		...
		//win
		if(st->async!=NULL)	{
			//printf("New zistem! saved.\n");
			st->async->GetConfig(msg);	
		}

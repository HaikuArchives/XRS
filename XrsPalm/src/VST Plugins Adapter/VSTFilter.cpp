	...
	fSampleRate (kDefaultFrameRate), fBlockSize (kDefaultBlockSize),  fIdleSem (-1), active(true)
{
	...
	/*else
		printf ( "The addon could not be loaded: %s\n", strerror (fImage));*/
...
void VSTFilterPlugin::load_plug_ins (const char *rootdir, BListView *list,int line)
{
								...
								{	// Chances are now that this is a VST plugin, but is it supported?...
									...
									if (effect && effect->magic == kEffectMagic)
									{
											...
											&& effect->numInputs <= 2)
										{
											...
											//list->AddItem (new MyItem(entry->d_name));
											//printf("Vst : %s\n",entry->d_name);

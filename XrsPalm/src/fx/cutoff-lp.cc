...
CutFilter::CutFilter()
{
    ...
    for (int i = 0; i <2; i++) {
    	...
    	chBuf[i]->x[0] = chBuf[i]->x[1] = 0;
	chBuf[i]->y[0] = chBuf[i]->y[1] = 0;
    }


}

float CutFilter::getSample(float inputSample)
{
    float outputSample;
...
//    if (!input->getSample(inputSample))
 //   	return 0;
    ...
    outputSample = (float)(a[0] * inputSample + a[1] * buf->x[0] + a[2] * buf->x[1]
